...
'''
    ...
    for word, relevance in zip(words, relevances):
        ...
        # Handle newline characters (Ċ is the newline token)  
        if word.strip() in ['Ċ', 'ĊĊ']:
...
def clean_tokens(words):
    ...
    # else: no special tokenization scheme, tokens are already clean
    
    # Escape special LaTeX characters for all tokenization schemes
    # Note: backslash must be first to avoid double-escaping
    special_characters = ['\\', '&', '%', '$', '#', '_', '{', '}']
    for i, word in enumerate(words):
        for special_character in special_characters:
            if special_character in words[i]:
                words[i] = words[i].replace(special_character, '\\' + special_character)
    
    # Handle ^ and ~ separately as they need special treatment
    for i, word in enumerate(words):
        if '^' in words[i]:
            words[i] = words[i].replace('^', '\\textasciicircum{}')
        if '~' in words[i]:
            words[i] = words[i].replace('~', '\\textasciitilde{}')
